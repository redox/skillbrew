...
1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." Do not create or modify any files in the audited repository — including `plans/`, `advisor-plans/`, plan README indexes, or any plan/spec Markdown — **unless the user explicitly asks for written plan files.** By default, put implementation plans into the host agent's **built-in planning capability** (plan mode or equivalent). If no built-in plan capability exists, present complete plans in chat. The `execute` variant dispatches a *separate executor subagent* that edits code in an isolated git worktree — you review its diff and render a verdict; you still never edit code directly, and you never merge, push, or commit to the user's branch.
...
5. **If the user asks you to implement directly, decline and point at the plan** — offer `execute <task>` (dispatched executor + your review) or plan refinement instead.
...
Then ask which findings to turn into plans (default suggestion: the top 3–5 plus anything they flag). Also surface **dependency ordering** — e.g. "characterization tests for module X (task 2) must land before the refactor of X (task 5)."

Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively (no user available to choose), plan the top 3–5 by leverage and record that default in the active plan summary (or note it in chat).

### Phase 4 — Build the plans

For each selected finding, add one plan task using the structure in [references/plan-template.md](references/plan-template.md) — read it before writing the first plan. **Default destination: the host agent's built-in planning capability** — one task per finding, with the full template content as the task body. If no built-in plan capability exists, present each complete plan in chat.
...
Before writing anything: record `git rev-parse --short HEAD` — every plan stamps the commit it was written against (the executor uses it for drift detection). If an active plan already exists from a previous run, **reconcile, don't duplicate**: review existing tasks, keep numbering monotonic, skip findings already planned or listed as rejected, and mark superseded tasks stale. If the user explicitly asked for written plan files, use `plans/` (or `advisor-plans/` when `plans/` already exists for an unrelated purpose) and say so.
...
Finish the active plan with a summary covering recommended execution order, dependencies between tasks, and findings considered and rejected. Only write `plans/README.md` or plan Markdown files when the user explicitly requested written plan files.
...
- `plan <description>` → skip the audit; the user already knows what they want. Run Recon, investigate just enough to specify it properly, and add a single plan task. If the description is too ambiguous to specify honestly, first try to resolve each ambiguity from the codebase itself; only what's left becomes questions to the user — asked one at a time, each with a recommended answer.
- `review-plan <task>` → critique an existing plan task in the active plan (or plan content the user pasted) against the template's standards and tighten it. If you authored the plan in this same session, also have a fresh-context subagent read it cold and report ambiguities — self-critique misses gaps you mentally fill from context the executor won't have.
- `execute <task>` → dispatch a cheaper executor subagent on one plan task (isolated worktree), then review its diff like a tech lead — re-run done criteria, check scope, read the code — and render a verdict. Treat the executor's diff as untrusted until reviewed: verify every hunk traces to a plan step and reject any out-of-scope change, however plausible it looks. Pass the **full plan task text** directly in the executor prompt — never assume the executor can read the active plan. Requires a host agent that can spawn subagents in an isolated worktree; if yours can't, say so and hand the plan over for manual execution instead. **Read [references/closing-the-loop.md](references/closing-the-loop.md) before the first dispatch.**
- `reconcile` → process what happened since last session: verify DONE tasks, investigate BLOCKED ones, refresh drifted TODOs, retire dead findings. See [references/closing-the-loop.md](references/closing-the-loop.md).
- `--issues` (modifier on any planning invocation) → also publish each plan as a GitHub issue via `gh`, with the issue URL noted on the plan task. Only with the explicit flag. Construct issue bodies from plan content directly — no temporary plan file required. **Before creating any issue, check whether the repo is public (`gh repo view --json visibility`). If it is, warn the user that issues are publicly visible and get explicit confirmation before publishing any plan that describes a security vulnerability, credential location, or other sensitive finding.** See [references/closing-the-loop.md](references/closing-the-loop.md).
