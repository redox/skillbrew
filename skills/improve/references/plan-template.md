...
1. **Self-contained context** — everything needed is in the task body: paths, code excerpts, conventions, commands.
...
Put each plan into the host agent's **built-in planning capability** as a separate task, numbered in recommended execution order. If no built-in plan capability exists, present the full structure below in chat. Only write plan Markdown files when the user explicitly asks for files.

---

## Structure

Use this structure for each plan task body:
...
- **Effort**: S | M | L
- **Risk**: LOW | MED | HIGH
- **Depends on**: <other plan task title or number, or "none">
...
After all plan tasks are written, include a summary in the active plan (or in chat if no built-in plan exists):

- **Execution order** — numbered list of plan tasks in recommended order.
- **Dependency notes** — e.g. "Task 2 requires Task 1 because <reason>."
- **Findings considered and rejected** — e.g. "<finding>: not worth doing because <one line>." (So nobody re-audits it.)

## Quality bar — check before finishing each plan

- Could a model that has never seen this repo execute this with only the plan task text and the repo? If any step requires knowledge from the advisor session, inline that knowledge.
...
- No secret values anywhere in the plan — locations and credential types only.
