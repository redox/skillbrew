...
## `execute <task>` — dispatch and review

### Preconditions (check all before dispatching)

- The repo is a git repository (worktree isolation requires it). If not: stop and say so.
- The plan task exists in the active plan (or was provided by the user) and its dependencies show DONE. If not: stop, name the missing dependency.
...
1. **The full plan task text, inlined.** Executors do not inherit the active plan — never assume they can read it. Always pass the complete task body directly.
...
> if a verification failed or was skipped, say so plainly. When finished, reply
...
| Verdict | When | Action |
|---|---|---|
| **APPROVE** | Criteria pass, scope clean, quality holds | Update the plan task status to DONE in the active plan. Present to the user: diff summary, worktree path and branch, anything from NOTES. **Merging is the user's decision — never merge, push, or commit to their branch.** |
...
| **BLOCK** | STOP condition hit, scope violated unrecoverably, or revisions exhausted | Mark BLOCKED on the plan task with the reason. Refine or rewrite the plan with what was learned. Tell the user what happened and what changed in the plan. |
...
## `reconcile` — keep the active plan alive

Process what happened since the last session. Read the active plan (or plan content the user provides) and review each task, then per status:

- **DONE** — spot-check that the done criteria still hold on the current HEAD (cheap ones only). Mark verified in the active plan. Keep completed task content — it's the record.
- **BLOCKED** — read the reason. Investigate the underlying obstacle in the codebase. Either rewrite the plan task around it (new task if the approach changed fundamentally, in-place refresh otherwise) or mark REJECTED with one line of rationale.
...
If the user explicitly maintains written plan files, read those instead and apply the same logic per file.
...
1. Preflight: `gh auth status` succeeds and the repo has a GitHub remote. If either fails, finish the plans in the active plan (or chat) as normal and say why issues were skipped.
...
4. Per plan: construct the issue body from the plan task content and run `gh issue create --title "<plan title>" --body "<constructed body>"`. Labels: `improve` plus the category — apply only if the labels exist or can be created without erroring; skip labels rather than fail. Do not write a temporary plan file just to use `--body-file`.
5. Record each issue URL in the plan task metadata (`- **Issue**: <url>`).

The plan task content is the source of truth; the issue is distribution. The self-containment rule pays off here — the issue body needs no edits to make sense to whoever (or whatever) picks it up.
